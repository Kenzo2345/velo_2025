...
def parse_args():
    p = argparse.ArgumentParser(description="Parallel parameter sweep (threads)")
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=n_workers) as ex:
        for run_id, row in df_params.iterrows():
            futures.append(ex.submit(_one_run, run_id, row))
    ...
    if args.plot:
        ...
        plt.title("Mailly / Moulin / Balance (threads)")
